...
class Net(nn.Module):
    ...
    def __init__(self, n_in, n_mid, n_out):
        ...
        # Dueling Network
        self.fc3_adv = nn.Linear(n_mid, n_out)  # Advantage部分
    ...
    def forward(self, x):
        ...
        adv = self.fc3_adv(h2)  # 输出不是ReLU
        val = self.fc3_v(h2).expand(-1, adv.size(1))  # 输出不是ReLU
        # 将[minibatchx1]的大小扩展为[minibatchx2]以计算val和adv的和
        # adv.size(1)是要输出的动作类型数量中的第二个

        output = val + adv - adv.mean(1, keepdim=True).expand(-1, adv.size(1))
        # val+adv中减去adv的平均值
        # 将adv.mean(1, keepdim=True) 在列方向(动作类型方向)上求平均，大小为[minibatch×1]
        # expand扩展为size[minibatchx2]
